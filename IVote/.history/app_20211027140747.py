...
def forgot():
    ...
    if request.method=="POST":
        ...
        c = conn.cursor()
        if "admin" in session:
            c.execute("select password from admin where email ='admin@gmail.com'")
            r = c.fetchall()
            if str(old) == str(r[0]) :
                c.execute("update admin where email ='admin@gmail.com' set password='"+str(new)+"'")
                return  render_template('forgotPassword.html',msg="updated")
            
        if "voter" in session:
            c.execute("select password from voter where cardno='"+session["voterid"]+"'")
            r = c.fetchall()
            if str(old) == str(r[0]) :
                c.execute("update voter where cardno set password='"+str(new)+"'")
                return  render_template('forgotPassword.html',msg="updated")
            return  render_template('login.html')
        conn.commit()
        c.close()
...
def genresult():
    """ This method generate result of any election
    """
    if "admin" in session:
        msg=""
        if request.method=="POST":
...
def viewblockchain():
    """dfisplay the content in the blockchain file
    """
    conn = sqlite3.connect("ivote.db")
    c = conn.cursor()
    c.execute("SELECT electionid FROM election")
...
def Vote_give():
    """ storin the votes in the databases and 
    """
    if request.method=="POST":
        id = request.form['id']
    ...
    return render_template('Voting.html',msg="welcome")
@app.route("/showelectionvoter<int:id>")
def showelectionvoter(id):
    """ show elections for voters
    """
    if "voter" in session:
        p=str(id)
        conn = sqlite3.connect("ivote.db")

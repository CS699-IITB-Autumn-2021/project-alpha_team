...
def Vote():
    if request.method=="POST":
        ...
        conn = sqlite3.connect("ivote.db")
        c = conn.cursor()
        c.execute("CREATE TABLE IF NOT EXISTS election"+id+"(secret_code TEXT ,name_of_blockchain TEXT,voter_id TEXT,vote_given TEXT)")
    
    return render_template('voter.html',allbc=allbc) 

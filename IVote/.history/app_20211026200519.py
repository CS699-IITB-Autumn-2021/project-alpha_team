...
def results():
    if request.method=="POST":
        ...
        conn = sqlite3.connect("ivote.db")
        c = conn.cursor()
        c.execute("SELECT * from result WHERE election_id = '"+str(id)+"'")
        r = c.fetchall()
        if len(r) >= 1:
            return redirect(url_for("showresultvoter",id = id))
...
def Vote_give():
    if request.method=="POST":
        ...
        conn = sqlite3.connect("ivote.db")
        c = conn.cursor()
        c.execute("CREATE TABLE IF NOT EXISTS election"+str(id2)+"(secret_code TEXT ,name_of_blockchain TEXT,voter_id TEXT,vote_given TEXT)")
        r1 = random.randint(0, 100000)
        voter_id=1
        rs=c.execute("select * from election"+str(id)+ " where voter_id='"+str(voter_id)+"'")
        B=Block(0,0,0,0)
        if(len(rs)==0):
            ...
            return render_template('Voting.html',r1=r1,hash=hash) 

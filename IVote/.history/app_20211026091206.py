...
c = conn.cursor()
c.execute("delete from election where electionid='Ashwin Patidar'")
...
c.execute("CREATE TABLE IF NOT EXISTS result(election_cand_id Text, noofvotes Number)")
...
def signup():
    if request.method=="POST":
        ...
        if password==confirm:
            conn = sqlite3.connect("ivote.db")
            c = conn.cursor()
            c.execute("INSERT INTO Voters VALUES('"+name+"','"+email+"','"+cardno+"','"+password+"','F')")
            conn.commit()
            conn.close()

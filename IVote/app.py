    ...
    if "admin" in session:
        if request.method=="POST":
            ...
            if confirm == new:
                ...
                r = c.fetchall()
                if old == r[0][0]:
                    c.execute("update admin set password='"+new+"' where email = '"+email+"'")
                    conn.commit()
                    c.close()
                    return redirect(url_for("admin"))
        return  render_template('forgotPassword.html',msg="updated")
            
    elif "voter" in session:
        if request.method=="POST":
            old=request.form["old"]
            new=request.form["new"]
            confirm=request.form["confirm"]
            ...
            if confirm == new:
                conn = sqlite3.connect("ivote.db")
                c = conn.cursor()
                c.execute("select password from Voters where email = '"+email+"'")
                r = c.fetchall()
                if old == r[0][0]:
                    c.execute("update Voters set password='"+new+"' where email = '"+email+"'")
                    conn.commit()
                    c.close()
                    return redirect(url_for("voter"))
        return  render_template('forgotPassword.html',msg="updated")
